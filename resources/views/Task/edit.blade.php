<x-layout>
    ...
    <div class="mx-auto">
        ...
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" method="post" action="{{ route('tasks.update', ['id' => $project_id, 't_id' => $task->id]) }}">
            @csrf
            <div class="grid grid-cols-2 gap-x-6 md:mt-4">
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="title">Title</label>
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="title" name="title" type="text" placeholder="Enter Title" value="{{ $task->title }}">
                ...
                <div class="mb-4">
                    ...
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="state" name="state" value="{{ $task->state }}">
                        <option value="">Select State</option>
                        @foreach($states as $state)
                        <option value="{{ $state }}" {{ $state ==  $task->state ? 'selected' : '' }}>{{ $state }}</option>
                        @endforeach
            ...
            <div class="grid grid-cols-2 gap-x-6 md:mt-4">
                <div class="mb-4">
                    ...
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="assigned_to" name="assigned_to">
                        ...
                        <option value="{{ $member->employee_id }}" {{ $member->employee_id == $task->assigned_to ? 'selected' : '' }}>{{ $member->name }} | {{ $member->role }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="priority" name="priority">
                        ...
                        <option value="{{ $prio}}" {{ $prio ==  $task->priority ? 'selected' : '' }}>{{ $prio }}</option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-2 gap-x-6 md:mt-4">
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="effort" name="effort" type="number" placeholder="Enter Effort (Hours)" value="{{ $task->effort }}">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="target_date" name="target_date" type="date" placeholder="Enter Target Date" min="{{ date('Y-m-d') }}" value="{{ $task->target_date }}">
            ...
            <div class="grid grid-cols-2 gap-x-6 md:mt-4">
                <div class="mb-4">
                    ...
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="risk" name="risk">
                        ...
                        @foreach($priorities as $prio)
                        <option value="{{ $prio}}" {{ $prio ==  $task->risk ? 'selected' : '' }}>{{ $prio }}</option>
                        @endforeach
                    </select>
